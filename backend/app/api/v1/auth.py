...
def login(request: Request, credentials: CustomerLogin, db: Session = Depends(get_db)):
    """
    Login com email/código e senha.
    Retorna JWT token para autenticação.
    """
    try:
        customer = auth_service.authenticate_customer(db, credentials.identifier, credentials.password)
    ...
    except auth_service.InactiveAccountError as e:
